 ...
 * defined in com.sleepycat_ustore_4117.je.config.EnvironmentParams and can be
 * represented by a property described by the EnvironmentConfig String
 * constants. Environment parameters have some interesting twists because there
 * are some attributes that are scoped by handle, such as the commit durability
 * (txnSync, txnNoSync, etc) parameters. DbConfigManager is instantiated first
 * by the EnvironmentImpl, and is loaded with the base configuration parameters.
 * If replication is enabled, additional properties are added when the
